...
- PDF text keeps its line breaks (`pageTextFromItems` in the loader uses pdf.js's `hasEOL`). Joining items with `""` glued lines into words like "SkillsProgramming", which embedded badly and gave the recursive chunker no newlines to split sections on; PDFs ingested before that fix should be re-uploaded.
...
- `.docx` keeps Word headings: `mammoth.convertToHtml` then the hand-rolled `htmlToMarkdown` (Heading 1–6 → `#`…, `<li>` → `- `). Not mammoth's markdown mode (deprecated, escapes text like `1\. Item`). Fixtures: `data/eval-corpus.docx` (Heading styles) / `data/sample-plain.docx` (none), from `scripts/gen-sample-docx.mjs`.
...
- `chunkSemantic(text, { similarityThreshold })` — embedding-based topic-boundary detection. Calls `embedBatch()` to embed every sentence, then splits where cosine similarity between consecutive sentences drops below `similarityThreshold` (default 0.75). **Requires Ollama to be reachable** — this is the only chunker strategy that makes network calls. Tests that exercise it use `test.skipIf(!ollamaUp)` to skip cleanly when the stack is down. Significantly more expensive than fixed/recursive; prefer `recursive` for general use. Measured in `scripts/compare-chunking.ts`: it cuts only at `.`/`!`/`?`, so the handbook becomes 48 scraps and bullet lists glue together; resume 3/14 R@1 vs recursive+auto 8/14. Not the default.
- **Heading-split fix (always on, `fixHeadingSplit`):** the recursive separators include bare `"## "`, so a long section used to be cut inside its own marker (`"#"`-only chunks, headings with no body; 61 of 208 chunks in the large corpus). The fix keeps each heading attached to its text (large corpus 208 → 117 chunks, same scores).
- **Title header, automatic (`INGEST_CONTEXT_HEADERS` = `auto` default / `1` / `0`):** a document with fewer than 2 `#` headings (`hasHeadingStructure()`, i.e. PDFs, plain notes, resumes, Word files without Heading styles) gets "Title › Section" embedded above every chunk; `payload.text` stays raw, the header is stored as `payload.section`. Structured documents get no header (it cost one refusal each on the handbook/large sets). Decided by the text, not the file extension. Resume R@1 5/14 → 8–9/14; every other gate unchanged (SESSION-LOG §25). Changing this changes vectors, so re-ingest.
- Measured and removed (SESSION-LOG §23–§24, code deleted): #2 bare-line heading detection, #3 800-char chunks + small-leftover merge (regressed structured docs: handbook refusals 0 → 6), and Anthropic-style Contextual Retrieval (an LLM blurb per chunk; ~25–30s per chunk on CPU, and one vague blurb pushed its chunk out of the top 5). `scripts/compare-chunking.ts` compares baseline / fix / 1+fix / auto / semantic.
...
- **Same-document expansion (dense):** the threshold decides *whether* a question has relevant material (it was calibrated on each question's best chunk). Once the best chunk clears it, the remaining topK slots are filled with that same document's next-best chunks, whatever their score; other documents still need 0.63. Without it, "What are Sekharendu Dey's projects?" matched the resume at 0.763 via the chunk naming him, and the project sections (0.50–0.53, no name in them) never reached the model. Hallucination test stayed 10/10 (the two near-miss questions now get 5 handbook chunks and still refuse).
...
- **Context assembly (`src/retrieval/context.ts`, `contextConfig` in `src/config.ts`):** retrieval decides *whether* and *which documents*; this decides how much of each the model reads. A matched document of ≤ `WHOLE_DOC_MAX_TOKENS` (1000 llama3 tokens, counted with `llama3-tokenizer-js`, never characters) goes in whole; a longer one gives its matched chunks ± 1 neighbour, in reading order, touching chunks merged. Documents fill `contextBudget()` (`src/rag.ts`: pinned `num_ctx` 4096 − system prompt − instruction − history − question − 512 answer reserve) in rank order; the best document always gets its best chunk. `RETRIEVE_EXPAND_CONTEXT=0` turns it off. A chunked document also gets its chunk #0 (title/byline, so "who wrote it" works; `CONTEXT_TITLE_CHUNK=0` off) and is capped at `CHUNKED_DOC_MAX_TOKENS` (1200): hits by score first, then the title chunk, then neighbours (SESSION-LOG §27). Measured paired old-vs-new with `scripts/check-answers.ts` (SESSION-LOG §26): resume fixture 17 → 22/25 correct ("how many projects" counts projects, not jobs), handbook and large corpus tie, big PDFs tie, one dilution loss on a 48-page PDF; answers ~2× slower on chunked documents, faster on whole small ones. Big-PDF misses were retrieval misses (title/byline chunk ranked 84th–113th for "who are the authors"), not context. Production-style check (one answer per question, random sample of 52 + 10 user-style questions, read by hand): 61/62 correct (98%; ~96–97% counting the known retrieval misses), refusals 14/14, median 48 s per answer; the misses left are retrieval (exact terms such as "GPT-4.1", slangy wording like "the CUGA thing"), not answering.
...
- **Follow-ups that refer back are rewritten first** (`src/generation/rewrite.ts`). If the question contains a referring word (he/his/it/that/those/ones/…) or opens with "And…"/"What about…" (`needsRewrite`), llama3 rewrites it into a standalone question ("tell me more about him" → "Tell me more about Sekharendu Dey."). Retrieval then runs on that question with the normal single 0.63 gate, like a first question. Guard rails, because llama3 alone is unreliable here (its first probe turned "What is the capital of France?" into "What was his employment period?"): standalone questions are never sent to the rewriter; refusals are dropped from its history; `cleanRewrite` rejects output that loses the question's own content words or is far longer (an answer). The answer prompt still uses the user's own words plus history. Measured (`scripts/evaluate-followups.ts`): two-turn follow-ups rank-1 10 → 11/13, 3-turn pronoun chains 2 → 4/4, off-topic leaks 0/6; ~6–8s per rewrite when warm (27s cold). `QUERY_REWRITE=0` disables it; `OLLAMA_REWRITE_MODEL` picks the model.
- **Fallback (no referring word, rewrite off or failed):** `retrieveForQuestion` passes the latest earlier user message as `previousQuestion` to `retrieve()`, which embeds `previous + "\n" + current` for the search.
- **Two gates for the fallback path:** the combined query must clear `RETRIEVE_SCORE_THRESHOLD` (0.63) as usual, and the current question on its own must clear `RETRIEVE_FOLLOWUP_FLOOR` (0.57, just above the 0.567 general-knowledge max from calibration). Without the floor an off-topic follow-up borrows the previous question's relevance (capital of France: 0.479 alone, 0.682 combined). Don't remove it.
...
- The system prompt also says never to answer a "who/what is" question with just a name: describe the person/thing with the key facts in the context. Generation pins `num_ctx` (`OLLAMA_NUM_CTX`, 4096) so the context budget matches the window Ollama actually loads.
...
Answer quality, old vs new context (`scripts/check-answers.ts`) — retrieves each question once and has llama3 answer from the plain top chunks (old) and from the assembled context (new), so differences come from the context alone. Grades by keywords (`must` groups; for eval-set files, the numbers/names in the expected text, else ≥50% of its content words; `refuse` entries must refuse) and flags "the context" leaks. Resumable: appends to `data/answer-check-<run>.jsonl`, reruns skip done questions. Read the changed verdicts by hand: the grader counts terse-but-correct answers as wrong. Uses temp document-store/sparse-stats paths, so `--ingest` into a throwaway collection never touches `data/documents.json`.
```bash
npx tsx scripts/check-answers.ts --run handbook --collection rag --set data/eval-set.json
npx tsx scripts/check-answers.ts --run handbook --summary                   # tally only
npx tsx scripts/check-answers.ts --run pdfs --collection tmp-check --ingest a.pdf,b.pdf --set q.json
```
...
Docker's WSL2 VM gets half the RAM (7.64 GB): long llama3 runs get the runner OOM-killed ("model runner has unexpectedly stopped", `docker inspect` shows `OOMKilled=true`); resumable scripts just rerun. `memory=10GB` in `%USERPROFILE%.wslconfig` + `wsl --shutdown` would stop it.
...
- `tests/rag.test.ts` — `answerQuestion` answerable + absent-topic (single) + **absent-topic STRESS (3 cases)** + follow-ups with history (vague follow-up answered, off-topic follow-up refused) end-to-end; the stress test is the multi-question vitest-side counterpart of `scripts/test-hallucination.ts` (kept short to stay under the vitest per-test timeout on CPU llama3). Its ingest pins `contextHeaders: false`: `sample.txt` has one heading, so auto would add a header that dilutes its single chunk below the follow-up floor, and this suite tests memory, not headers
